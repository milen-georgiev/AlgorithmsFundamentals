package ExerciseGraphTheoryTraversalAndShortestPaths.AreasInMatrix;
...
public class Main {
    ...
    private static void dfs(int row, int col, char areaSymbol) {
        ...
        if (isinOfBouns(row, col + 1) && !visited[row][col + 1] && matrix[row][col + 1] == areaSymbol) {
        ...
        if (isinOfBouns(row + 1, col) && !visited[row + 1][col] && matrix[row + 1][col] == areaSymbol) {
    ...
    private static boolean isinOfBouns(int row, int col) {
        return !isOutOfBouns(row, col);
    }

    private static boolean isOutOfBouns(int row, int col) {
